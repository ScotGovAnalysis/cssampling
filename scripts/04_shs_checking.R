...
if(length(list.files(path = shs.path,
                     pattern = "shs.frameandmatchedsample")) == 0){
  stop(print("03_shs_sampling.R needs to be run first."))
}
...
recent_frameandmatchedsample <- css_most_recent_file(path = shs.path, 
...
urbrur.la.qa <- css_check_urbrur(shs.frameandmatchedsample)
...
contractor.stream.la.qa <- css_check_stream(sample = contractor.sample,
                                        grouping_variable = la)
...
contractor.stream.urbrur.qa <- css_check_stream(sample = contractor.sample,
                                            grouping_variable = la,
...
cover <- rbind(cover,
               ...
               contractor.stream.la.qa[[1]],
               contractor.stream.urbrur.qa[[1]],
...
qa <- list(cover = cover,
           ...
           contractor.stream.la = contractor.stream.la.qa[[2]],
           contractor.stream.urbrur = contractor.stream.urbrur.qa[[2]],
